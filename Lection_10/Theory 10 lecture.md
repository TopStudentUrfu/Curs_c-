...
Наследование - Отношения между 2 классами. 
- Закрытый раздел родительского класса в классе наследника - недоступен!
- Protected - штука доступная родительскому классу наследнику, но недоступная из вне.
Побочные эффекты наследования
Возможность повторения используемого кода. (вызов открытых функций родительского класса)
Совместимость интерфейсов. (Сохраняет все те же самые функции (открытые), что есть в родительском классе). 

Полиморфизм-возможность во время выполнения программы предоставлять вместо 1 объекта другой объект с совместным интерфейсом.
Фактически место родительского объекта всегда можно подставить любой из унаследованных.
Плюсы ссылочного объявления: Delete не нужен. Уничтожиться после выхода из области видимости.

Виртуальные функции:
Функция в унаследованном классе может быть переопределена и использовано вместо базовой функции.
Virtual void eat()=0; абстрактная функция
Абстрактный класс-классы, у которых отсутствует реализация отдельного класса. Создать экземпляр отдельного класса нельзя. Абстрактный класс используется для определения интерфейса.
Деструктор-функция для очищения памяти. Деструкторы надо объявлять виртуальными.

Задание:
Придумать семейство объектов родительского. И как минимум 3 унаследованных от него. 
Интерфейс родительского класса минимум 3 функции. Конструктор + деструктор. 2 из них виртуальные.
Написать реализацию унаследованных объектах.
Маленькую программу-реализацию работы (создания экземпляры у наследования. Создать указатель или ссылку унаследовании. Показать, что если поменять, работает. В том числе полиморфизм.)

Паттерны проектирования
-Именует мотивиру и объясняет конкретный приём проектирования, который используется к задаче, часто возникает при работе над объектно Ориентированной системой. 
Патерн описывает задачу, которую надо решить, её решение, и результат, кроме того, он содержит рекомендации по её созданию. 
Решение реализации-схема организационных объектов и классов.
Преимущества использования патернов проектирования.
Выразительность и краткость
Возможность абстрагироваться от деталей реализации при проекте.
Единая номенклатура проектов и решений.
Паттерны-не библиотеки
